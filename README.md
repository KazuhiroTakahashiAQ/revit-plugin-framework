...
① new-plugin.bat をダブルクリック → プラグイン名・設定を入力
        ↓
② AIチャットに FOR_AI.md を渡してヒアリング → コード生成
        ↓
③ 生成された 2ファイルを plugins/<名前>/ に配置
        ↓
...
https://github.com/your-org/revit-plugin-framework
```

AIがヒアリングを進め、`PluginLogic.cs` と `PluginUI.cs` を生成します。
...
plugins/
└── WallColorizer/
    ├── PluginLogic.cs  ← AIが生成したファイルで上書き
    └── PluginUI.cs     ← AIが生成したファイルで上書き
```
